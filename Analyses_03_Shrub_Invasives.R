...
imls.spat$Mgmt.Vols <- as.factor(ifelse(imls.spat$PlotID %in% plot.vols, "Volunteers", "Other"))
summary(imls.spat)
...
shrub.2018 <- merge(shrub.2018, imls.spat[,c("PlotID", "x.nad83", "y.nad83", "x.utm16", "y.utm16", "lon", "lat", "PlotID2", "wooded", "unit", "MgmtUnit", "Mgmt.Vols")], all.x=T, all.y=T)
...
shrub.2007 <- merge(shrub.2007, imls.spat[,c("PlotID", "x.nad83", "y.nad83", "x.utm16", "y.utm16", "lon", "lat", "PlotID2", "wooded", "unit", "MgmtUnit", "Mgmt.Vols")], all.x=T, all.y=T)
...
shrub.bind <- rbind(shrub.2007[,c("Year", "PlotID", "MgmtUnit", "Mgmt.Vols", "Spp.Name", "Count", "Category")], 
                    shrub.2018[,c("Year", "PlotID", "MgmtUnit", "Mgmt.Vols", "Spp.Name", "Count", "Category")])
...
shrub.bind[grep("Ligustrum", shrub.bind$Spp.Name),"Invasive"] <- "Other Invasive"
shrub.bind[grep("Euonymus alatus", shrub.bind$Spp.Name),"Invasive"] <- "Other Invasive"
shrub.bind[grep("Rosa multiflora", shrub.bind$Spp.Name),"Invasive"] <- "Other Invasive"
...
shrub.bind$Mgmt.Vols[is.na(shrub.bind$Mgmt.Vols)] <- "Other"
summary(shrub.bind)

# unique(shrub.bind$Spp.Name)[order(unique(shrub.bind$Spp.Name))]
# shrub.bind[grep("Viburnum recognitum", shrub.bind$Spp.Name),]

shrub.agg <- aggregate(shrub.bind[,c("Count")], by=shrub.bind[,c("Year", "PlotID", "MgmtUnit", "Mgmt.Vols", "Invasive", "Invasive2")], FUN=sum)
...
shrub.agg2 <- tidyr::spread(shrub.agg[,c("PlotID", "Mgmt.Vols", "Invasive", "Invasive2", "Year", "Count")], Year, Count, fill=0)
...
ggplot(data=shrub.agg2[shrub.agg2$Invasive2=="invasive", ]) +
  ...
  geom_boxplot(aes(x=Invasive, y=Change, fill=Mgmt.Vols)) +
